...
> 📦 **Add-on · 2026-05-22** — **[WhatsApp Legacy Account Onboarding Spec v0.4](./whatsapp-old-account-onboarding/docs/README.md)** *(backup extractors)*: Port years of accumulated WhatsApp B2B sales conversations into an AI Agent's customer memory + style + history. One-command `bootstrap.sh` now decrypts iOS / Android backups inline — no iMazing or adb juggling. Three-layer architecture (MemOS + sales_playbook + conversation_history), A/B/C delivery decision tree, strict auto-onboard gate. [Release notes →](https://github.com/iPythoning/b2b-sdr-agent-template/releases/tag/whatsapp-onboarding-v0.4)
